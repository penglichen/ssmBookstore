...
<div class="container">
    ...
    <div class="row">
        <div class="col-7 column">
            <a class="btn btn-primary" href="${pageContext.request.contextPath}/order/toAddOrder">新增订单</a>
            <a class="btn btn-primary" href="${pageContext.request.contextPath}/order/allOrder1">显示所有订单</a>
        </div>
        <div class="col-5">
            <%--查询分类--%>
            <form class="form-inline" action="${pageContext.request.contextPath}/order/queryOrder" method="post" style="float: right">
                <span style="color:red;font-weight:bold;">${error}</span>
                <%--name属性是用于提交给后台的--%>
                <input type="text" name="queryorderByName" class="form-control" placeholder="请输入要查询的图书名称">
            ...
            <table class="table table-hover table-striped">
                ...
                <c:forEach var="order" items="${list}">
                    <tr>
                        ...
                        <td>${order.orderMount}</td>
                        <td>${order.priceStatus}</td>
                        <td>${order.receverAddr}</td>
                        <td>${order.receverName}</td>
                        <td>${order.receverTel}</td>
                        <td>
                            <a href="${pageContext.request.contextPath}/order/toOrder?id=${order.id}">修改</a>|
                            <a href="${pageContext.request.contextPath}/order/deleteOrderById/${order.id}">删除</a>
                        </td>
                    </tr>
                </c:forEach>
                <!-- 下面的一行是导航条 -->
